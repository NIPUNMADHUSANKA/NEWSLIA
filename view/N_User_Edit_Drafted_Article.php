...
<body>
    ...
    $variable_1 = "SELECT *FROM articles_drafted WHERE Post_Id = '$post_Id' ";
    ...
    if ($variable_3) {
        ...
        <form action='N_User_Pending_Posts.php' method='post' enctype='multipart/form-data'>
            ...
            <div style=' margin-bottom: 150px; margin-top: 60px; text-align: end; margin-right: 50px; display: flex; justify-content: flex-end;'>
                <input type='submit' value='Draft' class='write-article-btn' style='background-color: #45ADA8EB;' name='Draft_Drafted_Article' >
                <input type='submit' value='Send' class='write-article-btn' style='background-color: #00B172EB;' name='Pend_Drafted_Article' >
                <div onclick=togglePopup_select_option('confirm_delete'); class='write-article-btn' style=' background-color: #FF4444EB; text-align:center; '>Delete</div>
            </div>
        </form>
    </div>
